...
import { fetchOrgDomains, OrgDomain, STATUS_OPTIONS } from '@/api/orgDomains';
...
export default function OrgDomainList() {
  ...
  const { intervalSeconds, setIntervalSeconds } = useAutoRefresh('org-domains', load);

  const filtered = items.filter((d) => {
    const q = search.toLowerCase();
    const matchSearch = !q || d.name.toLowerCase().includes(q) || d.id.toLowerCase().includes(q) || d.owner.toLowerCase().includes(q);
        ...
        <div className="relative px-6 py-8 max-w-[1400px] mx-auto">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">Организационный домен</h1>
                <p className="mt-1.5 text-primary-foreground/65 leading-relaxed max-w-xl">
                  Перечень организационных доменов для работы с организационной структурой компании.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              />
              <button
                onClick={() => navigate('/org-domain/new')}
      ...
      <div className="px-6 py-6 max-w-[1400px] mx-auto">
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center text-muted-foreground gap-3">
            <div className="size-14 rounded-full bg-muted flex items-center justify-center">
              <Icon name="Building2" size={26} className="text-muted-foreground/50" />
            </div>
            <p className="font-medium">{items.length === 0 ? 'Нет доменов' : 'Ничего не найдено'}</p>
            {items.length === 0 && (
              <button
                onClick={() => navigate('/org-domain/new')}
        ...
        {!loading && filtered.length > 0 && (
          <div className="rounded-lg border border-border bg-card overflow-hidden">
            {/* Table header */}
            <div className="grid grid-cols-[1fr_160px_160px_100px_80px] gap-4 px-4 py-2.5 border-b border-border bg-muted/40 text-[11px] uppercase tracking-widest text-muted-foreground font-medium">
            ...
            {filtered.map((d, i) => (
              <Link
                key={d.id}
                to={`/org-domain/${d.id}`}
                className={`grid grid-cols-[1fr_160px_160px_100px_80px] gap-4 px-4 py-3.5 items-center hover:bg-muted/40 transition-colors ${i < filtered.length - 1 ? 'border-b border-border' : ''}`}
              >
