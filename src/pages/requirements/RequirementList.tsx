...
import { fetchRequirements, Requirement, STATUS_OPTIONS, TYPE_OPTIONS } from '@/api/requirements';
    ...
    <div className="p-6 max-w-[1400px] mx-auto space-y-6">
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="text-center py-20 text-muted-foreground">
          <Icon name="ListChecks" size={40} className="mx-auto mb-3 opacity-20" />
          <p className="text-sm">
            {search || statusFilter || typeFilter ? 'Ничего не найдено' : 'Требований пока нет'}
          </p>
          {!search && !statusFilter && !typeFilter && (
            <Link to="/requirements/new" className="mt-3 inline-flex items-center gap-1.5 text-sm text-accent hover:underline">
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-border bg-muted/40">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">Краткое описание</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground w-44 hidden md:table-cell">Тип</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground w-32 hidden lg:table-cell">Статус</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground w-20 hidden xl:table-cell">Версия</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground w-16 hidden xl:table-cell text-center">Балл</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground w-16 hidden xl:table-cell text-center">Вес</th>
                <th className="w-10" />
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {filtered.map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="font-medium truncate max-w-xs">{r.shortDesc || '—'}</div>
                    {r.tags.length > 0 && (
                  ...
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${TYPE_STYLE[r.reqType] || 'bg-muted text-muted-foreground'}`}>
                      {r.reqTypeLabel}
                    </span>
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell">
                    <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${STATUS_STYLE[r.status] || 'bg-muted text-muted-foreground'}`}>
                      {r.statusLabel}
                    </span>
                  </td>
