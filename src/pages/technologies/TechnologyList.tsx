...
import { fetchTechnologies, Technology, STATUS_OPTIONS } from '@/api/technologies';
...
export default function TechnologyList() {
  ...
  const { intervalSeconds, setIntervalSeconds } = useAutoRefresh('technologies', load);
  ...
  const filtered = items.filter((d) => {
    ...
    const matchSearch = !q
      ...
      || d.tags.some((t) => t.name.includes(q));
        ...
        <div className="relative px-6 py-8 max-w-[1400px] mx-auto">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">Технологии</h1>
                <p className="mt-1.5 text-primary-foreground/65 leading-relaxed max-w-xl">
                  Каталог технологий с описаниями, тегами, схемами и файлами.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              >
                <Icon name="Plus" size={16} /> Добавить технологию
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="px-6 py-6 max-w-[1400px] mx-auto">
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center text-muted-foreground gap-3">
            <div className="size-14 rounded-full bg-muted flex items-center justify-center">
              <Icon name="Cpu" size={26} className="text-muted-foreground/50" />
            </div>
            <p className="font-medium">{items.length === 0 ? 'Нет технологий' : 'Ничего не найдено'}</p>
            {items.length === 0 && (
              <button
                onClick={() => navigate('/technologies/new')}
        ...
        {!loading && filtered.length > 0 && (
          <div className="rounded-lg border border-border bg-card overflow-hidden">
            <div className="grid grid-cols-[1fr_150px_170px_90px_60px] gap-4 px-4 py-2.5 border-b border-border bg-muted/40 text-[11px] uppercase tracking-widest text-muted-foreground font-medium">
            ...
            {filtered.map((d, i) => (
              <Link
                key={d.id}
                to={`/technologies/${d.id}`}
                className={`grid grid-cols-[1fr_150px_170px_90px_60px] gap-4 px-4 py-3.5 items-center hover:bg-muted/40 transition-colors ${i < filtered.length - 1 ? 'border-b border-border' : ''}`}
              >
