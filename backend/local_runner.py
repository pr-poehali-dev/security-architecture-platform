...
Каждая функция из /backend/<name>/index.py вызывается через handler(event, context),
где event — словарь с httpMethod, headers, queryStringParameters, body.

Маршруты:
  GET/POST/PUT  /org-domains/*    → backend/org-domains/index.py
  GET/POST/PUT  /tech-domains/*   → backend/tech-domains/index.py
  GET/POST/PUT  /technologies/*   → backend/technologies/index.py
  GET/POST/PUT  /requirements/*   → backend/requirements/index.py
  GET/POST/PUT  /decisions/*      → backend/decisions/index.py
  GET/POST/PUT  /hardening/*      → backend/hardening/index.py
  GET/POST/PUT  /arch-templates/* → backend/arch-templates/index.py
"""
...
# ── Эмуляция контекста cloud function ────────────────────────────────────────
...
def dispatch(fn_name: str):
    ...
    # Собираем event в формате cloud function
...
for fn in FUNCTIONS:
    fn_name = fn  # замкнуть переменную
